...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    ...
    return questions.filter(question => question.body !== "" || question.expected !== "" || question.options.length !== 0);
}
...
export function sumPublishedPoints(questions: Question[]): number {
   ...
   let filterd: Question[] = questions.filter(question => question.published)

    return filterd.reduce((acc, question) =>  acc + question.points,0)
...
export function addNewQuestion(
    ...
    let copy = [...questions];
    return copy.concat(makeBlankQuestion(id,name,type));
}
...
export function duplicateQuestionInArray(
    ...
    const targetIndex = copy.findIndex(question => question.id === targetId);

    if (targetIndex === -1) {
        return copy; 
    }

    const duplicatedQuestion = duplicateQuestion(newId, copy[targetIndex]);

    copy.splice(targetIndex + 1, 0, duplicatedQuestion);
